...
from src.models.loss_functions import physics_informed_sequence_loss  # use physics loss via trainer
...
def main():
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        train_loss = trainer.train_one_epoch(train_loader, teacher_forcing_ratio=tf_ratio)
        val_loss = trainer.validate_one_epoch(val_loader)
        print(f"Epoch {epoch}/{EPOCHS} TF={tf_ratio:.2f}  Train Loss={train_loss:.4f}  Val Loss={val_loss:.4f}")

    # Save via trainer
    trainer.save_model('checkpoints/seq_physics.pth', data['scalers'], params={
