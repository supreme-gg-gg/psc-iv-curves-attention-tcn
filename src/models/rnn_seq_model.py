...
class RNNIVModel(IVModelBase):
    ...
    Implements a bidirectional GRU with layer normalization and residual connections.
    ...
    Args:
        ...
        hidden_dim (int): Hidden dimension for GRU and MLP layers.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.voltage_points = torch.tensor(
            voltage_points
        ...
        # GRU: Bidirectional GRU with residual connections
        self.gru = nn.GRU(
            input_size=2,
        ...
        # Layer normalization for GRU outputs
        ...
        self.current_head = nn.Linear(
            hidden_dim, 1, bias=True
        )  # Predict single value per timestep
    ...
    def _rnn_step(self, input_token, hidden):
        """
        Single GRU step: returns prediction, EOS logit, and updated hidden.
        """
        ...
        return pred, eos, hidden

    def forward(
        self, physical, target_seq=None, lengths=None, teacher_forcing_ratio=0.5
    ):
        """
        ...
        # Training mode: if full teacher forcing, use parallel GRU; else scheduled sampling loop
        if target_seq is not None:
            max_len = target_seq.size(1)

            if teacher_forcing_ratio == 1.0:
                # fast parallel teacher forcing with optional packing
                x = torch.cat(
                    [
                ...
                if lengths is not None:
                    # pack, run GRU, then pad back
                    packed = rnn_utils.pack_padded_sequence(
                        x, lengths.cpu(), batch_first=True, enforce_sorted=False
                    )
                    packed_out, _ = self.gru(packed, hidden)
            ...
            teacher_mask = (
                torch.rand(batch_size, max_len, device=device) < teacher_forcing_ratio
            )
            ...
            for t in range(max_len):
                # run one step
                pred, eos, hidden = self._rnn_step(input_token, hidden)
                ...
                if teacher_mask[:, t].all():
                    ...
                    next_curr = torch.where(
                        teacher_mask[:, t].view(batch_size, 1, 1),
                        gt,
                        pred.unsqueeze(-1),
                    )
                ...
                v_next = (
                    volt_seq[:, t + 1 : t + 2]
                    if t + 1 < volt_seq.size(1)
                    else torch.zeros_like(init_token)
                )
        ...
        # Start with the cached init_token and first voltage
        input_token = torch.cat([init_token, volt_seq[:, 0:1]], dim=-1)
        ...
        for t in range(max_len):
            # run one step
            next_token, next_eos, hidden = self._rnn_step(input_token, hidden)
            ...
            # attach next voltage (or zero) using actual volt_seq length to avoid empty slices
            v_next = (
                volt_seq[:, t + 1 : t + 2]
                if t + 1 < volt_seq.size(1)
                else torch.zeros_like(init_token)
