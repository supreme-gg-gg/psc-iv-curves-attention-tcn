...
def main():
    ...
    X_test_tensor, y_test_tensor, test_mask_tensor, test_length, eos_targets_test = (
        data["test"]
    )
    ...
    total_trainable_params = sum(
        p.numel() for p in cvae_model.parameters() if p.requires_grad
    )
    ...
    for epoch in range(1, training_epochs + 1):
        ...
        avg_test_loss = trainer.validate_one_epoch(test_loader, kl_beta=kl_beta_current)
