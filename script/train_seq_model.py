...
from src.utils.loss_functions import sequence_loss_with_eos
...
def main():
    ...
    train_dataset = TensorDataset(
        X_train, padded_y_train, mask_train, lengths_train, eos_train
    )
    # test does not require EOS targets
    test_dataset = TensorDataset(
        X_test, padded_y_test, mask_test, lengths_test, eos_test
    )
    ...
    trainer = IVModelTrainer(
        ...
        loss_params={"eos_loss_weight": EOS_LOSS_WEIGHT},
    )
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        train_loss = trainer.train_one_epoch(
            train_loader, teacher_forcing_ratio=current_tf_ratio
        )
        val_loss = trainer.validate_one_epoch(test_loader)
        print(
            f"Epoch {epoch}: Train Loss = {train_loss:.4f}, Val Loss = {val_loss:.4f}, TF Ratio = {current_tf_ratio:.4f}"
        )
    ...
    # Evaluate
    mean_r2, samples = trainer.evaluate(
        test_loader, data["scalers"], include_plots=True
    )
